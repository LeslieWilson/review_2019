...
def location(array)
  array.each do |hash|
    hash.each do |key, value|
      puts "#{key} is in #{value["location_1_city"]}, #{value['location_1_state']}"
    end
  end
end
...
def type(array)
  array.each do |hash|
    hash.each do |key, value|
      puts "#{key}: #{value["location_1"]["type"]}"
    end
  end
end
